...
func (h *Handler[V]) BRPop(args []string) server.RESPValue {
	if len(args) < 2 {
		return server.RESPValue{
			Type: server.Error,
			Str:  "ERR wrong number of arguments for 'brpop' command",
		}
	}
		...
		for _, key := range keys {
			...
			if found {
				...
				return server.RESPValue{
					Type: server.Array,
					Array: []server.RESPValue{
						{Type: server.BulkString, Str: key},
						{Type: server.BulkString, Str: val},
					},
				}
			}
		}
