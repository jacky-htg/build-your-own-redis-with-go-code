...
func (s *Server) processCommandWithTx(c *Connection, value *RESPValue) RESPValue {
	...
	if cmd == "PUBLISH" {
		return s.handlePublish(c, args)
	}

	if c.IsSubscribeMode() {
		if cmd == "UNSUBSCRIBE" {
			// UNSUBSCRIBE di-handle di handlePubsub
			return s.handleUnsubscribe(c, args)
		}
		return RESPValue{
			Type: Error,
			Str:  "ERR only UNSUBSCRIBE command allowed in subscribe mode",
		}
	}

	if cmd == "SUBSCRIBE" {
		return s.handleSubscribe(c, args)
	}
...
func (s *Server) handleConnection(conn net.Conn) {
	...
	for {
		...
		// Set read timeout
		if !c.IsSubscribeMode() && s.config.ReadTimeout > 0 {
